...
for(let i=0;i<Anum;i++){
    for(let j=i+1;j<=Anum;j++){
        a_sub.push(a_arr.slice(i,j).reduce(function add(s,c){return s+c;},0))
    }
}
for(let i=0;i<Bnum;i++){
    for(let j=i+1;j<=Bnum;j++){
        b_sub.push(b_arr.slice(i,j).reduce(function add(s,c){return s+c;},0))
    }
}
a_sub.sort(function (a,b){return a-b})
b_sub.sort(function (a,b){return b-a})
